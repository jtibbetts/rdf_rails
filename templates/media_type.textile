...
h1. #{@json['hydrax:docTitle']}

Final Version 2.0

Media Type

#{@json['hydrax:mediaType']}
...
2. "The #{ get(meta_type_name) } Media Type":#The_#{ get(meta_type_name) }_Media_Type
...
Figure 1 shows the representation of a #{media_type_name} resource in the @#{media_type_format}@ format.
...
Figure 1.  Example JSON document containing a #{media_type_name} object
...
JSON-LD specifies four other reserved terms (@value, @language, @container, @list). Ordinarily, these terms are not used in the JSON binding for @#{media_type_name}@ objects. However, implementations that extend this specification by including additional properties may utilize these reserved terms in accordance with the rules defined by "[JSON-LD-syntax]":#_JSON_LD_syntax_.

h3. 1.3 The JSON-LD Context

In JSON-LD, a context is used to map simple names that appear in a JSON document to URI values for properties or data types in a formal vocabulary (typically an RDF ontology). For example, the standard context "[LTI, 14 IG]":#_LTI__14_IG_ for a #{media_type_name} contains the following rewrite rules (among others):
...
h2. 2. The #{media_type_name} Media Type

The following list defines the necessary and sufficient conditions for a document to conform to the @#{media_type_format}@ media type.
...
# The root object must have a <tt>@type</tt> property whose value is "@#{media_type_name}@".
...
Figure 6 presents a complete graphical representation of the JSON binding for a #{media_type_name} object. The subsections following this figure provide details about each object that appears in the JSON binding for a #{media_type_name} object.
